...
.PHONY: build
build:
	docker build $(DOCKER_OPT_ARGS) --build-arg GIT_USERNAME=$(GIT_USERNAME) --build-arg GIT_PASSWORD=$(GIT_PASSWORD) --build-arg PROJECT=$(PROJECT) --build-arg ARTICLE_GRAPHQL_ENDPOINT=$(ARTICLE_GRAPHQL_ENDPOINT) --build-arg ARTICLE_GRAPHQL_TOKEN=$(ARTICLE_GRAPHQL_TOKEN) --build-arg LOG_LEVEL=$(LOG_LEVEL) --build-arg NODE_ENV=$(NODE_ENV) -t $(FULL_NAME) .
